...
@TargetApi(18)
  ...
  private final DefaultDrmSessionManager.OnSaveKeyResponseInMediaDrm onSaveKeyResponseInMediaDrm;
...
//  }
  ...
  public DefaultDrmSession(
      ...
      EventDispatcher<DefaultDrmSessionEventListener> eventDispatcher,
      LoadErrorHandlingPolicy loadErrorHandlingPolicy,
      DefaultDrmSessionManager.OnSaveKeyResponseInMediaDrm onSaveKeyResponseInMediaDrm) {
    if (mode == DefaultDrmSessionManager.MODE_QUERY
        || mode == DefaultDrmSessionManager.MODE_RELEASE) {
      Assertions.checkNotNull(offlineLicenseKeySetId);
    }
    ...
    try {
      ...
      if (mode == DefaultDrmSessionManager.MODE_RELEASE) {
        ...
        if (keySetId != null && keySetId.length != 0) {
          if (onSaveKeyResponseInMediaDrm != null) {
            onSaveKeyResponseInMediaDrm.onSaveKeyResponse(keySetId);
          }
        }
        //endregion xxxBADxxx
        state = STATE_OPENED_WITH_KEYS;
